...
CREATE TABLE pdp_piece_mh_to_commp (
    ...
    commp text NOT NULL,
    -- NB(piri): v1 CommP CID string for the same piece. The pdpv0 tables and
    -- the prove task's piece reader are v1-native (Curio convention), while
    -- piri's mapping is keyed by the v2 form; this column lets the resolver
    -- answer v1 lookups (S3PieceReader.GetSharedPieceReader) without needing
    -- the raw size to reconstruct the v2 CID.
    commp_v1 text NOT NULL
);
...
--
-- NB(piri): sub_piece_offset is part of the key. Piri aggregates blobs into
-- roots and stages one row PER SUBROOT, so all of a root's rows share
-- (add_message_hash, add_message_index) — Curio's piece-per-message key —
-- and only the offset disambiguates them. Same reason the downstream
-- pdp_data_set_pieces PK is (data_set, piece_id, sub_piece_offset), and the
-- same shape piri's pre-Curio pdp_proofset_root_adds PK had.

ALTER TABLE ONLY pdp_data_set_piece_adds
    ADD CONSTRAINT pdp_data_set_piece_adds_pk PRIMARY KEY (add_message_hash, add_message_index, sub_piece_offset);
...
-- NB(piri): v1-form lookups (see commp_v1 column comment).
CREATE INDEX pdp_piece_mh_to_commp_commp_v1_idx ON pdp_piece_mh_to_commp USING btree (commp_v1);
