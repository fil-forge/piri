...
// /blob/unallocate is performed by the upload service under a
...
func (s *RPCSuite) TestBlobUnallocate_ParkedBlobReleased() {
	t := s.T()
	...
	proof := testutil.Must(delegation.Delegate(
		s.ServiceID, s.UploadServiceIdentity.DID(), service, blob.Unallocate.Command,
	))(t)
	unalloc := testutil.Must(blob.Unallocate.Invoke(
	...
	// Idempotent.
	again := testutil.Must(blob.Unallocate.Invoke(
		s.UploadServiceIdentity,
		service,
		&blob.UnallocateArguments{
			Space:  space,
...
func (s *RPCSuite) TestBlobUnallocate_AcceptedBlobRefused() {
	t := s.T()
	service := s.ServiceID.DID()
	...
	_, err := blob.Unallocate.Unpack(rcpt)
