...
type Manager struct {
	...
	queue       jobqueue.Service[*aatodo_types.Aggregation]
...
func (m *Manager) Submit(ctx context.Context, aggregateLinks ...cid.Cid) error {
	...
	for len(remaining) > 0 {
		...
		if len(batch) == maxBatchSize {
			...
			if err := m.doSubmit(aatodo_types.Aggregation{Roots: batch}); err != nil {
...
func (m *Manager) doSubmit(aggregates aatodo_types.Aggregation) error {
	...
	if err := m.queue.Enqueue(m.ctx, m.taskHandler.Name(), &aggregates); err != nil {
